...
function setLoteria(loteria) {
    ...
    // IMPORTANTE: Aguardar o select ser atualizado antes de mostrar cartões
    setTimeout(() => {
        const selectConcurso = document.getElementById('concursoSelect');
        if (selectConcurso && selectConcurso.options.length > 1) {
            // Forçar o select para o primeiro concurso
            selectConcurso.selectedIndex = 1;
            console.log(`📌 Select atualizado, concurso: ${selectConcurso.value}`);
        }
