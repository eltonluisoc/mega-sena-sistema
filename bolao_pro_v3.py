...
Correções v6.17 (Importar Extrato reconstruído — 5ª das "5 sugestões"):
 - Aba "📥 Importar Extrato" de volta, refeita do zero com um PDF real
   ...
   a extração de texto corrido quebrava com acentuação, e o pdfplumber
   nunca esteve de fato empacotado em nenhum .exe publicado).
 - Extração agora é POSITION-AWARE (coordenadas x/y de cada palavra na
   página, mesma técnica já usada no import de cartões), não texto
   corrido — funciona mesmo com o bug de encoding de fonte do PDF do
   Nubank (letra acentuada vira "�" no extract_text(), mas nomes em
   CAIXA ALTA e valores continuam legíveis, e a posição na página não
   depende da acentuação). Funções novas em nível de módulo:
   _extrair_creditos_pix_extrato() e _extrato_match_participante()
   (casamento só quando há UM candidato certo — ambíguo ou zero vira
   "sem correspondência" pra revisão manual, nunca adivinha).
 - Tela nova: escolhe o PDF, analisa (roda em thread, não trava a UI),
   mostra cada crédito Pix encontrado com o participante sugerido
   (verde = pronto, laranja = sem participante, cinza = já importado
   antes — checa duplicata por participante+valor+data). Duplo-clique
   troca o participante manualmente. Nada é lançado sem estar
   selecionado na lista e confirmado no botão "Importar Selecionados".
 - Mês de referência do pagamento importado vem da DATA DO PIX no
   extrato, não da data de hoje (extrato antigo = pagamentos de meses
   passados). Log de auditoria registra o total importado por PDF.
 - SistemaBoloes.spec: pdfplumber + dependências em hiddenimports
   (garante que desta vez é empacotado de verdade). Achado no processo:
   o analisador do PyInstaller incluía numpy/pandas/scipy/pyarrow por
   causa de um caminho de código do pdfplumber que nunca é chamado
   aqui (extract_table, não usado — só extract_words) — inflava o .exe
   de ~32MB pra >115MB à toa. Confirmado com um teste bloqueando esses
   imports deliberadamente que extract_words() funciona sem eles, e
   excluídos do build — .exe final fica em ~36MB.
